define(['text!pages/role/role.html', 'pages/role/rolemeta', 'css!pages/role/role', 'uuitree','uuigrid'], function (html) {
  ...
  var init = function(element,params){
		...
		var viewModel = {
				...
				/* 角色数据  -- 列表 */
				roleData : new u.DataTable(rolemeta),

				/* 角色编辑数据 -- 信息录入 */
				roleDatanew : new u.DataTable(rolemeta),
				...
				event:{
					...
					loadRoleList:function(org){
						...
						var dtVal = viewModel.roleData.getValue('rolename');
    ...
    offset: '60'
                ...
                showValue: function (obj) {
					...
					addroleClick:function(){
						...
						$('#dialog_content_man').find('.u-msg-title').html("<h4>新增角色</h4>");
						viewModel.event.clearDt(viewModel.roleDatanew);
							var newr = viewModel.roleDatanew.createEmptyRow();
							viewModel.roleDatanew.setRowSelect(newr);
					...
					editroleClick:function(){
						...
						//2. 反显  --  要编辑的数据
						//3. 保存
						$('#dialog_content_man').find('.u-msg-title').html("<h4>编辑角色</h4>");
						viewModel.event.clearDt(viewModel.roleDatanew);
						var row = viewModel.roleData.getSelectedRows()[0]
						if(row){
