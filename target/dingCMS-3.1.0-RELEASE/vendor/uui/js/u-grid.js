...
/***/ function(module, exports) {
	...
	var init = function init(options, gridComp) {
	    ...
	    var gridDefault = {
	        ...
	        canSaveSet: gridOptions.canSaveSet||false,
	    };
...
/***/ function(module, exports, __webpack_require__) {
	...
	var re_createColumnMenu = function re_createColumnMenu() {
		...
		htmlStr += '</div></li>';
		//保存配置
        if(this.options.canSaveSet){
            htmlStr += '<li class="u-grid-column-menu-li" role="menuitem">';
            htmlStr += '<div class="u-grid-column-menu-div1 c_saveset" id="' + this.options.id + '_saveSet">';
            htmlStr += '<span class="u-grid-column-menu-span">' + this.transMap.ml_save_set + '</span>';
            htmlStr += '</div></li>';
        }
	...
	var colMenu_initGridEventFun = function colMenu_initGridEventFun() {
		...
		// 保存设置按钮
		$('#' + this.options.id + '_saveSet').off('click');
		$('#' + this.options.id + '_saveSet').on('click', function (e) {
			if (typeof oThis.options.onSaveSetFun == 'function') {
				oThis.options.onSaveSetFun();
			}
		});
		// 显示/隐藏列 对应所有列的点击处理
