define(['text!pages/procedure/procedure.html','css!pages/procedure/procedure','pages/procedure/proceduremeta','uuigrid'], function(template) {
    ...
    offset: '60'
                ...
                renderTypeAjax:function (obj) {
                    var id = obj.row.value.procedureid;
                    var text = obj.value;
                    var html = "<a style='text-decoration: underline;' class='c_green' href='#/procedure/procedureDetail?id="+ id +"'>" + text||'' + "</a>";
                    obj.element.innerHTML = html;
                },

                // 新增
                addClick:function () {
                  window.location.href = ctx + "/#/procedure/procedureDetail";
                },
                ...
                delClick:function () {
                    ...
                    if(row){
                        ...
                        var msgTitle = '删除确认';
                        var msgContent = "是否删除"+data.procedurename +"?";
                        $vue.$confirm(msgContent, msgTitle, {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning'
                        }).then(() => {
                            viewModel.event.delprocedure(data.procedureid);
                        }).catch(() => {
                            $vue.$message({
                                type: 'info',
                                message: '已取消删除',
