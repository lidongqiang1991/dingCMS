define(['text!pages/transferpay/transferpay.html', 'css!pages/transferpay/transferpay', 'pages/transferpay/transferpaymeta', 'uuigrid'], function(template) {
	...
	var init = function(element) {
		...
		var viewModel = {
			...
			objDelUrl : ctx + '',
			orderListUrl : ctx +'/order/list.do',

			objdata : new u.DataTable(productmeta),
			objdatanew : new u.DataTable(paymentmeta),
			// 筛选 数据
			searchdatanew : new u.DataTable(productmeta),
			//存放订单数据
			orderList : [],
			typeList : [{
				name : "全部类型",
    ...
    offset: '60'
				...
				renderTypeAjax : function(obj) {
					...
					var text = obj.value;
					var html = "<a style='text-decoration: underline;' class='c_green' href='#/transferpay/transferpayDetail?id=" + id + "'>" + text || ' ' + "</a>";
				...
				addClick : function(obj) {
					...
					$('#dialog_content_transferpay').find('.u-msg-title').html("<h4>新增转移支付</h4>");
					// $('#dialog_content_transferpay').find("#meno").val(null);
					viewModel.event.clearDt(viewModel.objdatanew);
					var row = viewModel.objdata.getSelectedRows()[0];
					if (row) {
					    //在点击弹出新增转移框，初始化  订单列表下拉框
					    viewModel.event.loadOrderInit(row.data.billid);
						...
						window.md = u.dialog({
							id : 'add_transferpay',
							content : "#dialog_content_transferpay",
							hasCloseMenu : true
						});
    ...
    offset: '60'
				...
				saveClick : function() {
					...
					// var memo=$('#dialog_content_transferpay').find("#meno").val();
					// data.memo=memo;
    ...
    offset: '60'
			    ...
			    //初始化订单列表
			    loadOrderInit:function(serviceid){
			        $.ajax({
                        type: "GET",
                        url: viewModel.orderListUrl,
                        data: {
                            serviceid : serviceid
                        },
                        success: function (res) {
                                 if (res) {
                                    ...
                                    var list = [{"name":"请选择订单","value":""}].concat(res||[]);
                                    var newlist = list.map(function(item) {
                                        if(item.formid){
                                            item.name = item.vbillcode;
                                            item.value = item.billid;
                                        }
                                        return item;
                                    });

                                    viewModel.orderList = newlist;
                                 } else {
