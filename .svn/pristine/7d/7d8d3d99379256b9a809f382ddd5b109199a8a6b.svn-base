define(['text!pages/form/form.html','css!pages/form/form','pages/form/formmeta','uuigrid'], function(template) {
    ...
    var init=function  (element) {
        var viewModel = {
            ...
            // 表单
            listUrl:ctx + '/form/list.do',
            delUrl:ctx + '/form/del.do',
            objdata:new u.DataTable(formmeta),
    ...
    offset: '60'
                ...
                renderTypeAjax:function (obj) {
                    var id = obj.row.value.formid;
                    var text = obj.value;
                    var html = "<a style='text-decoration: underline;' class='c_green' href='"+ctx+"/#/form/formDetail?id="+ id +"'>" + text||'' + "</a>";
                    obj.element.innerHTML = html;
                },

                // 新增
                addClick:function () {
                  window.location.href = ctx + "/#/form/formDetail";
                },
                // 删除表单  -- delform()
                delClick:function () {
                    ...
                    if(row){
                        ...
                        var msgTitle = '删除确认';
                        var msgContent = "是否删除"+data.formname +"?";
                        $vue.$confirm(msgContent, msgTitle, {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning'
                          }).then(() => {
                            viewModel.event.delform(data.formid);
                          }).catch(() => {
                            $vue.$message({
                              type: 'info',
                              message: '已取消删除',
